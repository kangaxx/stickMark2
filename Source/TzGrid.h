				...
				public ComboBox::Listener
{
	...
	void drawRow(int rowIdx, StringArray data)
	{
		if (rowsComponents.size() == getRowNum() * getColumNum())
			return; //´´½¨¿Ø¼þµÄ¹ý³ÌÖ»ÐèÒªÖ´ÐÐÒ»´Î£¬Èç¹û½ñºóÐèÒªÌí¼Óresize¹¦ÄÜÔòÐèÒªµ÷Õû
		//if (data == NULL)
		//	return;
		...
		for (int i = 0; i < columns.size(); i++)
		{
			...
			case columnComponentType::switchBox:
				...
				tmpCombo->addItem(TRANS(juce::CharPointer_UTF8("\xe5\x85\xb3\xe6\x9c\xba")), 1); //1 ¹Ø»ú
				tmpCombo->addItem(TRANS(juce::CharPointer_UTF8("\xe5\xbc\x80\xe6\x9c\xba")), 2);
				tmpCombo->setSelectedId(1); //Ä¬ÈÏ¹Ø»ú
...
private:
	...
	StringArray gridData; //ÖÇÄÜÖ¸ÕëÊý×é
	...
	bool initialed = false; //µÚÒ»´Îdraw¿Ø¼þÊ±»á¸ü¸Ä³õÊ¼»¯×´Ì¬
